...
@section('main')
  ...
  height: 60px;
    ...
    <form enctype="multipart/form-data" class="mp-container" id="rform" action="{{ route('edittkt') }}" method="post">
        @csrf
        <input type="hidden" name="date" value="{{$inv[0]->date}}" required>
        <input type="hidden" name="invoice" value="{{$inv[0]->invoiceid}}" required>
        ...
        <div id="cart" class="modal bg-under">
            <div class="modal-content bg-under">
                ...
                <div>
                    @foreach ($inv as $prod)
                    @php
                         $item = DB::table('products')->where('name', $prod->item)->first();
                    @endphp
                    <div style="margin-top: 5px;" class="mp-card" id="{{ $item->id . 'list' }}">
                        <div class="row" style="margin:3px; padding: 0;">
                            ...
                            <div class="col s6"><input type="number" id="{{ $item->id . 'listinp' }}" name="quantity[]"
                                    ...
                                    onfocusout="changequantity2({{ $item->id }})" value="{{$prod->quantity}}"></div>
                        </div>
                        <div style="margin: 0;">
                            <div class="center"><textarea type="text" id="{{ $item->id . 'listinpdet' }}" name="detail[]"
                                ...
                                onkeyup="changequantity2({{ $item->id }})"
                                onfocusout="changequantity2({{ $item->id }})" value="{{$prod->cusremarks}}">{{$prod->cusremarks}}</textarea></div>
                        </div>
        ...
        <div class="mp-card row prod" id="{{ $category }}" style="margin: 3px; padding: 10px;">
            ...
            <div class="col s9 row" style="padding: 0; margin: 0;">
                ...
                <div class="col s6"><input type="number" id="{{ $item->id . 'viewinp' }}" inputmode="numeric"
                        ...
                        onkeyup="changequantity({{ $item->id }})" value="{{$prod->quantity}}"></div>
                <div class="row col s12 price-line valign-wrapper"  style="padding: 0;  margin: 0;">
                    <div class="col s12"><textarea type="text" id="{{ $item->id . 'viewinpdet' }}" onkeyup="changequantity({{ $item->id }})" placeholder="Damage - Details"
                            class="browser-default inp" value="{{$prod->cusremarks}}">{{$prod->cusremarks}}</textarea></div>
                </div>
            </div>
